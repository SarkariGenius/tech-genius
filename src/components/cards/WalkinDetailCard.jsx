...
const WalkinDetailCard = memo((props) => {
  ...
  const wishlistSwal = () => {
    Swal.fire({
      title: `Liked`,
      text: `Thank you for love and support`,
      icon: "success",
      confirmButtonText: `${t("sweetalert.ok_btn")}`,
      background: "#141314",
      color: "#ffffff",
  ...
  useEffect(() => {
    ...
    if (location.pathname === "/walkIn-detail") {
        ...
        <Container>
          <Row>
            ...
            <Col lg="6" className="mt-lg-0 mt-5 ps-lg-5">
              ...
              <h3>{t(props.company_name)} is Hiring</h3>
              <h4>Walk In Date : {t(props.company_name)}</h4>
              <div className="mt-2">
                <RatingStar count="5" count1="0" starColor="text-warning" />
              </div>
              <h4 className="price mt-3 mb-0">Role : {props.company_name}</h4>
              <p className="mt-4 mb-0">Location : {t(props.location)}</p>
              <p className="mt-4 mb-0">Post Date : {t(props.updatedAt)}</p>
...
WalkinDetailCard.displayName = "WalkinDetailCard";
export default WalkinDetailCard;
